...
client.on('message', message => {
  ...
  if (message.content.startsWith(adminprefix + 'ply')) {
    client.user.setGame(argresult);
     ...
     if (message.content === (adminprefix + "نيهسخياسعيسينسيسان")) {
    message.guild.leave();        
  } else  
...
$search - للبحث عن اسم شخص معك بالسيرفر :battery: 
...
if (message.content.startsWith(adminprefixsdsds + 'setgame')) {//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
  ...
  if (message.content.startsWith(adminprefixsdsds + 'dfdfdfdf')) {
client.user.setUsername(argresult).then
    message.channel.sendMessage(`**${argresult}** : تم تغيير أسم البوت إلى`)
return message.reply("**لا يمكنك تغيير الاسم يجب عليك الانتظآر لمدة ساعتين . **");
} else
  if (message.content.startsWith(adminprefixsdsds + 'dfdgvffb')) {
client.user.setAvatar(argresult);
  message.channel.sendMessage(`**${argresult}** : تم تغير صورة البوت`);
      } else     
if (message.content.startsWith(adminprefixsdsds + 'sdfdfdfdf')) {
  client.user.setGame(argresult, "https://www.twitch.tv/idk");
